...
import '../../../Models/PendingJobs/PendingJobsModel.dart';
import '../../../Utilities/Colors/colors.dart';
import 'pendingCustomerDetails.dart';

class PendingJobsComplainsUI extends StatefulWidget {
  const PendingJobsComplainsUI({super.key});

  @override
  State<PendingJobsComplainsUI> createState() => _PendingJobsComplainsUIState();
}

class _PendingJobsComplainsUIState extends State<PendingJobsComplainsUI> {
  List<PendingJobsModel> pendingJobList = [];
  List<PendingJobsModel> searchPendingList = [];
  bool loading = true;
  ...
  Widget _buildBottomSheet(BuildContext context, PendingJobsModel model) {
  ...
  Future Post_InstalledTechnicianJob() async {
    var response = await http.get(Uri.parse(GetPendingJobs));
    ...
    if (response.statusCode == 200) {
      ...
      for (Map i in result) {
        pendingJobList.add(PendingJobsModel.fromJson(i));
      }
      setState(() {
        searchPendingList = List.from(pendingJobList);
      });
    } else {
      setState(() {
  ...
  void search(String query) {
    setState(() {
      searchPendingList = pendingJobList.where((category) {
